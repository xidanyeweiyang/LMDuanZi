...
+ (void)initialize {
    ...
    // 设置导航栏背景颜色
    [UINavigationBar appearance].barTintColor = LMNavigationBarTintColor;
    // 设置导航栏标题文字颜色
    // 创建字典保存文字大小和颜色
    NSMutableDictionary * color = [NSMutableDictionary dictionary];
    color[NSFontAttributeName] = LMFont(17.0f);
    color[NSForegroundColorAttributeName] = LMNavigationTittleColor;
    [[UINavigationBar appearance] setTitleTextAttributes:color];
    
    // 拿到整个导航控制器的外观
    UIBarButtonItem * item = [UIBarButtonItem appearance];
    item.tintColor = LMNavigationItemTintColor;
    // 设置字典的字体大小
    NSMutableDictionary * atts = [NSMutableDictionary dictionary];
    atts[NSFontAttributeName] = [UIFont systemFontOfSize:17];
    atts[NSForegroundColorAttributeName] = LMNavigationItemTintColor;
    // 将字典给item
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if (self.viewControllers.count > 0) {// 如果现在push的不是栈底控制器(最先push进来的那个控制器)
        ...
        btn.tintColor = LMNavigationItemTintColor;
        ...
        viewController.navigationItem.leftBarButtonItem = [UIBarButtonItem lm_barButtonWithTitle:@"" touchBlock:^{
